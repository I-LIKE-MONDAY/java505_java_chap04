...
//        사용자가 원하는 결과를 얻기 위한 소스코드를 한 번 입력할 때는 문제가 없지만 여러 번 사용할 경우 소스코드의 중복이 발생함

//        매개변수 : 해당 메서드 내의 연산 시 필요한 메이터를 외부에서 전달하는 변수.
...
//           - 연산의 결과가 입력되는 데이터에 따라 변경됨(데이터를 외부에서 전달받기때문)
        ...
        sumType1();     // 두 수의 덧셈은 : 30   <-- 결과 고정
        subType2(10,40);    // 두 수의 뺄셈은 : -30   <-- 매개변수가 있으므로 결과 고정X, 넣는 값에 따라 달라짐
        subType2(100,200);
